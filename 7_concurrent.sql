...
-- 실습 절차
-- 1) 워크벤치에서 auto_commit 해제. update 후, commit 하지 않음. (transaction1)
-- 2) 터미널 열어 select 했을 때 위 변경사항이 읽히는 확인(transaction2)
-- 결론 : mariadb는 기본이 repeatable read이므로 dirty read 발생하지 않음 

-- read committed : 커밋한 데이터만 read 가능 -> phantom read 발생(or non-repeatable read)
start transaction;
...
-- repeatable read : 읽기에 일관성 보장 -> Lost Update 문제 발생 -> 배타적 잠금으로 해결 
-- lost update 문제 발생
DELIMITER // 
...
-- serializable : 모든 트랜잭션 순차적 실행 -< 동시성 문제 X(성능 저하)
