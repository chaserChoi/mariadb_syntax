-- pk, fk, unique 제약조건 추가 시에 해당 컬럼에 대해 index 페이지 자동 생성.
...
-- 만약 where 조건에서 2컬럼으로 조회 시에, 1컬럼에만 index가 있다면,
...
-- index는 1컬럼 뿐만 아니라, 2컬럼 대상으로 1개의 index를 설정하는 것도 가능.
-- 이 경우 두 컬럼을 and 조건으로 조회해야만 index 사용 
-- 복합 인덱스 생성
